...
import { 
  ...
  AppsButton, Divider, Label, LoadingIndicator, Barcode, 
  I18n,
  AppConfig,
...
export default class CouponTNCView extends React.Component {
  ...
  componentDidMount(){
    ...
    this.setState({
      ...
      redeemStatus: (
        (coupon_detail.total_used_count < coupon_detail.member_limit_count || coupon_detail.member_limit_count == 0) && 
        `${coupon_detail.valid_from} ${coupon_detail.time_from}`< coupon_detail.worldDateTime && 
        `${coupon_detail.valid_to} ${coupon_detail.time_to}` > coupon_detail.worldDateTime &&
    ...
    this.getBalanceUsage(coupon_detail.member_limit_count, coupon_detail.total_used_count);
  }
  ...
  handleRenderCouponImage(coupon_primary_color, coupon_secondary_color){
    // Coupon Valid Date
    var valid_from = (this.state.coupon_detail.valid_from && !this.state.coupon_detail.register_coupon) ? `FROM ${this.state.coupon_detail.valid_from} ${this.state.coupon_detail.time_from}` : this.state.coupon_detail.register_coupon ? `FROM ${this.state.coupon_detail.register_coupon_valid_from}` : ``;
    var valid_to = (this.state.coupon_detail.valid_to && !this.state.coupon_detail.register_coupon) ? `${this.state.coupon_detail.valid_to} ${this.state.coupon_detail.time_to}` : this.state.coupon_detail.register_coupon ? this.state.coupon_detail.register_coupon_valid_to : ``;
    ...
    return(
      <View style={{
        ...
        <View style={{marginVertical: Metrics.baseMargin}}>
          <Divider lineColor={coupon_primary_color} text={"Discount"} textBold={true} textColor={coupon_primary_color}/>
        </View>

        {/* Coupon Value */}
        <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderBottomWidth: 1, borderColor: coupon_primary_color, paddingBottom: Metrics.basePadding}}>
          <Label
            text={(this.state.coupon_detail.discount_by == 'amt')?`${AppConfig.prefix_currency}`:''}
            type="normal_bold" 
            style={{color: coupon_primary_color, fontSize: Fonts.size.h1}}
          />
          <Label
            text={`${this.state.coupon_detail.value}${(this.state.coupon_detail.discount_by == 'per')?'%':''}`}
            type="normal_bold"
            style={{color: coupon_primary_color, fontSize: Fonts.size.h1}}
          />
        </View>
        ...
        <View style={{
          ...
          <Image source={Images.company_logo} style={{width: '100%', height: Metrics.couponHeight, resizeMode: 'contain', opacity: 0.2}} />
        </View>
      </View>
    )
  }
    ...
    return(
      <View style={{padding: Metrics.basePadding}}>
        ...
        <View style={{marginBottom: Metrics.baseMargin}}>
          <Text style={{fontSize: Fonts.size.h4, fontWeight: 'bold'}}>
          {
            this.state.coupon_detail.barcode ?
            this.state.coupon_detail.barcode.replace(this.state.coupon_detail.barcode.substring(2,14), "************") :
            ``
          }
          </Text>
        </View>
    ...
    return(
      <View>
        <Text style={{fontSize: Fonts.size.h6, fontWeight: 'bold'}}>{`Redeem Status`}</Text> 
        {
          ...
          ?
          this.handleRenderRedeemStatusText(true, `Coupon is ready to be redeem.`)
          :
          <View>
            {/* Error - Reached Limit Usage */}
            {
              (this.state.coupon_detail.member_limit_count && this.state.coupon_detail.total_used_count == this.state.coupon_detail.member_limit_count)
              ?
              this.handleRenderRedeemStatusText(false, `Coupon have reached the limited usage.`)
              :
              <View/>
            }
            {/* Error - Validation Date */}
            {
              (`${this.state.coupon_detail.valid_from} ${this.state.coupon_detail.time_from}` > this.state.coupon_detail.worldDateTime)
              ?
              this.handleRenderRedeemStatusText(false, `Coupon able to use start from ${this.state.coupon_detail.valid_from} ${this.state.coupon_detail.time_from}.`)
              :
              <View/>
            }
            {/* Error - Expired Date */}
            {
              (`${this.state.coupon_detail.valid_to} ${this.state.coupon_detail.time_to}` < this.state.coupon_detail.worldDateTime)
              ?
              this.handleRenderRedeemStatusText(false, `Coupon expired at ${this.state.coupon_detail.valid_to} ${this.state.coupon_detail.time_to}.`)
              :
              <View/>
            }
            {/* Error - Registered Validation Failed */}
            {
              ...
              ?
              this.handleRenderRedeemStatusText(false, `Coupon only available once registered in Mobile App since Day ${this.state.coupon_detail.member_limit_mobile_day_start} to ${this.state.coupon_detail.member_limit_mobile_day_end}.`)
              :
              <View/>
            }
            {/* Error - Limit Profile Info Validation Failed */}
            {
              ...
              ?
              this.handleRenderRedeemStatusText(false, `Coupon require to complete member profile info (${this.state.coupon_detail.member_profile_info_require_item.toString()}).`)
              :
              <View/>
            }
          </View>
  ...
  handleRenderTNC(){
    // Coupon Valid Date
    var valid_from = (this.state.coupon_detail.valid_from && !this.state.coupon_detail.register_coupon) ? `${this.state.coupon_detail.valid_from} ${this.state.coupon_detail.time_from}` : this.state.coupon_detail.register_coupon ? this.state.coupon_detail.register_coupon_valid_from : ``;
    var valid_to = (this.state.coupon_detail.valid_to && !this.state.coupon_detail.register_coupon) ? `${this.state.coupon_detail.valid_to} ${this.state.coupon_detail.time_to}` : this.state.coupon_detail.register_coupon ? this.state.coupon_detail.register_coupon_valid_to : ``;
    
    return(
      <View style={{marginVertical: Metrics.baseMargin*2}}>
        <Text style={{fontSize: Fonts.size.h6, fontWeight: 'bold'}}>{`Terms and Conditions:`}</Text>

        {/* Condition in Valid Department Used */}
        {
          (this.state.coupon_detail.dept_desc)
          ?
          this.handleRenderTNCText(`This coupon can only be used in ${this.state.coupon_detail.dept_desc} department.`)
          :
          <View/>
        }
        ...
        {/* Condition in Valid Brand Items */}
        {
          ...
          ?
          this.handleRenderTNCText(`This coupon can only be used for ${this.state.coupon_detail.brand_desc} brand items.`)
          :
          <View/>
        }
        ...
        {/* Condition in Valid Vender*/}
        {
          ...
          ?
          this.handleRenderTNCText(`This coupon is only valid at ${this.state.coupon_detail.vendor_desc} vendor.`)
          :
          <View/>
        }
          ...
          <View>
            {
              ...
              ?
              this.handleRenderTNCText(`This coupon is only valid for ${this.state.description.toString()} product/s.`)
              :
              this.handleRenderTNCText(`Please connect to internet to get the specific product valid for the coupon redemption.`)
            }
          </View>
          :
          <View/>
        ...
        {this.handleRenderTNCText(`This coupon is valid starting from ${valid_from} until ${valid_to}.`)}

        {/* Minimum Purchase Quantity */}
        {
          (this.state.coupon_detail.min_qty)
          ?
          this.handleRenderTNCText(`Minimum purchase quantity: ${this.state.coupon_detail.min_qty}`)
          :
          <View/>
        }
        ...
        {/* Minimum Purchase Amount */}
        {
          ...
          ?
          this.handleRenderTNCText(`Minimum purchase amount: ${AppConfig.prefix_currency} ${this.state.coupon_detail.min_amt}`)
          :
          <View/>
        }
        ...
        {/* Minimum Spend in Receipt Amount */}
        {
          ...
          ?
          this.handleRenderTNCText(`Spend ${AppConfig.prefix_currency} ${this.state.coupon_detail.min_receipt_amt} and above in a single receipt to make you eligible for the coupon redemption.`)
          :
          <View/>
        }

        {/* Notice */}
        {this.handleRenderTNCText(`This coupon is unexchangeable for cash or any products.`)}
        ...
        {/* Balance Usage */}  
        {
          (this.state.coupon_detail.member_limit_count)
          ?
          this.handleRenderTNCText(`Balance usage: ${this.state.balance_usage}`)
          :
  ...
  handleRenderTNCText(text){
    return(
      <View style={{flexDirection: 'row', alignItems: 'flex-start', margin: Metrics.baseMargin, marginLeft: 0}}>
        <Image 
  ...
  render() {
    // Local variable
    const coupon_primary_color = this.state.redeemStatus ? Colors.primary : Colors.inactive_primary;
    const coupon_secondary_color = this.state.redeemStatus ? Colors.secondary: Colors.inactive_secondary;
    ...
    return(
      ...
      <SafeAreaView style={[ApplicationStyles.screen.safeAreaContainer, {backgroundColor: Colors.body}]} forceInset={{vertical:'never'}}>
        ...
        >
          ...
          {this.handleRenderCouponImage(coupon_primary_color, coupon_secondary_color)}
        ...
        <View style={{margin: Metrics.baseMargin}}>
          <AppsButton 
            ...
            onPress={() => {this.handleCouponBarcodeModalVisibleOnChanged(true)}}
        ...
        <Barcode
          ...
          onClose={(value) => {this.handleCouponBarcodeModalVisibleOnChanged(value)}}
