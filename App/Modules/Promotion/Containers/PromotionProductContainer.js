...
export default class PromotionProductView extends React.Component {
  ...
  static navigationOptions = ({ navigation, navigationOptions }) => {
    ...
    return {
      ...
      headerLeft: (
        <View>
          <TouchableOpacity style={tailwind("bg-white rounded-lg opacity-100 p-2 ml-3 mt-3")} onPress={() => navigation.navigate('PromotionScreen')}>
            <Image
            ...
            <Card containerStyle={tailwind("bg-white rounded-lg opacity-100")} >
              ...
              <View style={{ flexDirection: 'row',}}>
                ...
                <View style={PromotionContainer.descriptionContainer}>
                  ...
                  {/* Product Description */}
                  <View style={PromotionContainer.descriptionRowContainer}>
                    <Text ellipsizeMode={'middle'} numberOfLines={2} style={tailwind("text-base text-secondary")}>{item.sku_desc}</Text>
                  </View>

                  {/* Product Original Price */}
                  <View style={PromotionContainer.descriptionRowContainer}>
                    <Text style={tailwind("text-lg text-secondary")}>{`Original Price: `}</Text>
                    <Text style={tailwind("text-base text-primary font-bold")}>{`${prefix_currency} ${this.convertToCurrencyFormat(item.selling_price)}`}</Text>
                  </View>
                  ...
                  {/* Product amount discount */}
                  {
                    (item.display_amt)
                      ...
                      // Discount Amount
                      <View style={PromotionContainer.descriptionRowContainer}>
                        <Text style={tailwind("text-lg text-secondary")}>{`Discount Amount: `}</Text>
                        <Text style={tailwind("text-base text-primary font-bold")}>{`${prefix_currency} ${this.convertToCurrencyFormat(item.member_discount_amount)}`}</Text>
                      </View>
                      :
                      <View />
                  }
                  ...
                  {/* Product Fixed Price */}
                  {
                    (item.display_fixed_price)
                      ?
                      <View style={PromotionContainer.descriptionRowContainer}>
                        <Text style={tailwind("text-lg text-secondary")}>{`Promo Price: `}</Text>
                        <Text style={tailwind("text-base text-primary font-bold")}>{`${prefix_currency} ${this.convertToCurrencyFormat(item.member_fixed_price)}`}</Text>
                      </View>
                      :
                      <View />
                  }
          ...
          <View>
            <ScrollView
                ...
                (this.state.datalistSpecial.length > 0)
                  ...
                  <View>
                    <View style={[PromotionContainer.headerContainer, { backgroundColor: Colors.primary, width: '100%', marginLeft: 0, paddingHorizontal: Metrics.basePadding }]}>
                      <Text style={tailwind("text-primary text-xl font-bold")}>Special For You</Text>
                    </View>
              ...
              <View style={tailwind("mb-16")}>
                {/* Promotion Title */}
                <View style={tailwind("px-3 justify-center items-center")}>
                  <Text style={tailwind("text-primary text-xl font-bold")}>{this.state.promo_title_data.title}</Text>
                </View>
