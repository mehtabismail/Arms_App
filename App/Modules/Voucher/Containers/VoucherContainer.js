...
export default class VoucherView extends React.Component {
    ...
    return {
      ...
      headerTitle: (
        <View style={VoucherContainer.headerButtonContainer}>
          <TouchableOpacity style={{paddingVertical: Metrics.smallPadding}} onPress={() => navigation.navigate('CouponScreen', {loginUpdate: true})}>
            <Label 
              style={{
  ...
  componentDidUpdate(prevProps){
    ...
    // Voucher Update
    var voucher_update = this.props.navigation.getParam("voucher_update", false);
  ...
  async getWorldTimeData(){
    ...
    var worldDateTimeData = await worldTimeAPICommunicator.GetRealWorldTimeDateTime();
    var worldDateTime = worldDateTimeData ? moment(worldDateTimeData).format('YYYY-MM-DD HH:mm:ss') : moment().format('YYYY-MM-DD HH:mm:ss');
        ...
        <ScrollView 
          ...
          <View style={[ApplicationStyles.screen.mainContainer,{marginBottom: 100}]}> 
            ...
            <View style={[VoucherContainer.headerContainer]}>
              <Text style={[Fonts.style.h5, {fontWeight: 'bold', color: Colors.primary}]}>COLLECT NOW, SHOP NOW</Text>
              <Text style={{color: Colors.primary}}>FOR THE BETTER PRICE !!!</Text>
            </View>
  ...
  handleRenderEmptyDataScreen(){
    return(
      <View style={[ApplicationStyles.screen.mainContainer, {justifyContent: 'center', flex: 0, height: '100%', paddingBottom: 250}]}>
        ...
        <Label> . . . . . </Label>
        <Label style={{color: Colors.primary}}>Oops! No voucher have been issued yet.</Label>
      </View>
    )
  }
  ...
  handleFlatListRenderItem = ({item, index}) => {
    const redeem_status = item.active && !item.cancelled && !item.voucher_used && item.valid_from < this.state.worldDateTime && item.valid_to > this.state.worldDateTime;
    const voucher_primary_color = redeem_status ? Colors.primary : Colors.inactive_primary;
    const voucher_secondary_color = redeem_status ? Colors.secondary: Colors.inactive_secondary;
    
    return (
      <FadeInAnimation index={ index } >
        <TouchableOpacity  
          style={[
            VoucherContainer.shadow, {
        ...
        >
          ...
          <View style={{
            ...
            {/* Voucher Left Part */}
            {this.handleRenderVoucherLeftPart(item, voucher_primary_color, voucher_secondary_color)}
            
            {/* Voucher Right Part */}
            {this.handleRenderVoucherRightPart(item, voucher_primary_color, voucher_secondary_color)}
  ...
  handleRenderVoucherLeftPart(item, voucher_primary_color, voucher_secondary_color){
    return(
      <View style={{width: '65%', flex: 1}}>
        ...
        <View>
          <Label 
            text={`VOUCHER`}
            type={"normal_bold"}
            style={{color: voucher_primary_color, fontSize: Fonts.size.h4}} 
          />
        </View>
        ...
        <View>
          <Label 
            text={`VALID ${(item.valid_from)?`FROM ${item.valid_from}`:``} TILL ${(item.valid_to)?item.valid_to:``}`}
            type={"normal"}
        ...
        <View style={{flex: 1, justifyContent: 'space-evenly'}}>
          <Label type="normal_bold" style={{color: voucher_primary_color, fontSize: Fonts.size.large}}>{`${item.voucher_barcode}`}</Label>
          {
            (!item.active)
            ?
          ...
          }
          {
            (item.voucher_used)
            ?
            <View><Label type="normal" style={{color: voucher_primary_color, fontSize: Fonts.size.small}}>{`- Voucher used at ${item.used_time}`}</Label></View>
            :
            <View/>
          }
          {
            ...
            ?
            <View><Label type="normal" style={{color: voucher_primary_color, fontSize: Fonts.size.small}}>{`- Voucher able to use start from ${item.valid_from}`}</Label></View>
            :
            <View/>
          }
          {
            ...
            ?
            <View><Label type="normal" style={{color: voucher_primary_color, fontSize: Fonts.size.small}}>{`- Voucher expired at ${item.valid_to}`}</Label></View>
            :
            <View/>
          }
        </View>
  ...
  handleRenderVoucherRightPart(item, voucher_primary_color, voucher_secondary_color){
    return(
      <View style={{width: '35%', alignItems: 'flex-end', justifyContent: 'flex-end'}}>
        <View>
          <Label 
            ...
            type={"normal_bold"}
            style={{color: voucher_secondary_color, fontSize: Fonts.size.h6, alignSelf: 'flex-start'}} 
          />
          
          <Label 
            text={`${(item.voucher_value.toString().length==1)?` ${item.voucher_value}`:item.voucher_value}`}
            type="normal_bold" 
            style={{
              color: voucher_secondary_color,
              fontSize: (item.voucher_value.toString().length>2)?
  ...
  handlerRenderVoucherValueBackground(voucher_primary_color){
    return(
      ...
      <View style={{
        ...
        zIndex: -1, 
        width: '45%', height: 0, 
        borderLeftWidth: (Metrics.voucherHeight)/4, borderLeftColor: 'transparent', 
        borderBottomWidth: Metrics.voucherHeight, borderBottomColor: voucher_primary_color
      }}/>
    )
  }
